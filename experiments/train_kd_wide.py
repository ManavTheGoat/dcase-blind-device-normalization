...
def main():
    ...
    print(" FreqAttn (frozen) -> CPMobileWide(bc=40) + DC-KD")
    print("=" * 60)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if va > best_acc:
            best_acc = va
            torch.save(classifier.state_dict(), 'results/kd_wide_best_classifier.pt')

    print(f"\n{'='*60}")
    print(f"  DCASE 2025 baseline:        50.72%")
    print(f"  FreqAttn (no KD):           45.23%")
    print(f"  FreqAttn + DC-KD + Wide:    {best_acc*100:.2f}%")
    ...
    with open('results/kd_wide_log.json', 'w') as f:

...
from src.utils.augmentation import freq_mixstyle
...
def main():
    ...
    print(" FreqAttn (frozen) -> CPMobile + DC-KD + FMS")
    print("=" * 60)
    ...
    print(f"CPMobile params: {classifier.count_parameters():,}  (trainable)")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for mel, scene_lbl, device_lbl, idx in train_loader:
            ...
            t_logits   = teacher_logits_all[idx]  # (B, 10) — indexed by dataset position
        ...
        if va > best_acc:
            best_acc = va
            torch.save(classifier.state_dict(), 'results/kd_best_classifier.pt')

    print(f"\n{'='*60}")
    print(f"  DCASE 2025 baseline:        50.72%")
    print(f"  FreqAttn (no KD):           45.23%")
    print(f"  FreqAttn + DC-KD + FMS:     {best_acc*100:.2f}%")
    ...
    with open('results/kd_log.json', 'w') as f:
